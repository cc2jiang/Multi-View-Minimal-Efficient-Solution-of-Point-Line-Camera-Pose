function [T_est_all_my,time] = MV_P1P2L(delta_R,delta_t,P1_0,P2_0,L3_0,L4_0,R_w0_to_c0,t_w0_to_c0,T_w0_to_c0,noise,length)
    ...
    %% -----------------------------------------
    [t_y_est] = solve_functions(A,X_3,X_4,Y_3,Y_4,B,x);
    %% -----------------------------------------
    [R_all,t_all] = recover_T(A,X_3,X_4,Y_3,Y_4,B,t_y_est);
    
    %% 
    for i=1:4
        T_est_all_my(:,:,i)=inv(T_c0_to_c1)*[R_all(:,:,i),t_all(:,i);0 0 0 1]*inv(T_w1_to_w0);
    end
    T_est_all_my=real(T_est_all_my);
    time=toc;
end
